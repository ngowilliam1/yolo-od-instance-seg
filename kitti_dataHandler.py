...
def calculate_depth(imgDisparity, baseline, focal_length):
    depth = np.zeros_like(imgDisparity, dtype=np.float32)
    imgDisparityFloat = imgDisparity.astype('float32')
    # The following handles cases where the disparity = 0
    # Since the disparity will be set to be a very small number, the resulting depth will be large and thus filtered out
    # by the depth > 80 restriction
    imgDisparityFloat[imgDisparity < 0.0001] = 0.0001
    depth = focal_length*baseline/imgDisparityFloat
    depth[depth > 80] = 0
    depth[depth < 0.1] = 0
    return depth
